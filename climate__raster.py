# -*- coding: utf-8 -*-
# ---------------------------------------------------------------------------
# climate__raster.py
# Created on: 2018-01-10 10:04:44.00000
#   (generated by ArcGIS/ModelBuilder)
# Usage: climate__raster <CSV>  <X_Field> <Y_Field> <Z_value_field> <output>
# Description: This takes the outputs of the 6 GCM climate models and creates a raster for each variable within each CSV
# ---------------------------------------------------------------------------

# Import arcpy module
import arcpy
import os

print "imported"

#set workspace
arcpy.env.workspace = "R:\\Streletskiy Research\\Luis_Files\\Thesis\\GCM_Results\\CSVs\\2006_2050"
arcpy.env.overwriteOutput = True
print "workspace set"

#access CSVs and confirm 
files = arcpy.ListFiles("*.csv")
print files

#set variables and confirm
Z_value_field = ["ta", "h_snow", "fu", "magt", "altr", "altr_alt", "pr", "fdi", "ftc"]
X_Field = "lat"
Y_Field = "lon"
spRef  = "GEOGCS['GCS_WGS_1984',DATUM['D_WGS_1984',SPHEROID['WGS_1984',6378137.0,298.257223563]],PRIMEM['Greenwich',0.0],UNIT['Degree',0.0174532925199433]];-400 -400 1000000000;-100000 10000;-100000 10000;8.98315284119522E-09;0.001;0.001;IsHighPrecision"
xy_output = "temp_layer"
print Z_value_field
print "variables set"

#still having some issues with the save_file.... ".csv" at the end of file names is messing up the save, how do I truncate it?
def climate_raster(x,y):
	for xs in x:
		arcpy.MakeXYEventLayer_management(xs, X_Field, Y_Field, xy_output, spRef)
		for ys in y:
			#THIS is WHERE THE ISSUE IS. MUST TRUNCATE .csv AWAY FROM END OF xs FILES
			save_file = "R:\\Streletskiy Research\\Luis_Files\\Thesis\GCM_Results\\TIFFs\\2006_2050\\TEST_%s" %(ys)
			arcpy.gp.NaturalNeighbor_sa(xy_output, ys, save_file, "R:\\Streletskiy Research\\Luis_Files\\Thesis\\GIS\\Rasterization_Files\\1x1_standard")
			print "Done %s %s" %(xs, ys)
	print "Done all"

climate_raster(files, Z_value_field)
			
print "Process Complete"
	




